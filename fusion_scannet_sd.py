...
def get_args():
    ...
    parser.add_argument('--prefix', type=str, default='sd', help='prefix for the output file')
...
def process_one_scene(data_path, out_dir, args):
    ...
    img_dirs = sorted(glob(join(scene, 'color/*.jpg')), key=lambda x: int(os.path.basename(x).split('.')[0]))
    ...
    for img_id, img_dir in enumerate(tqdm(img_dirs)):
        ...
        feat_2d_0 = torch.nn.functional.interpolate(feat_2d[0], size=(240, 320), mode='bicubic', align_corners=False).squeeze(0) # 1280, 240, 320
        feat_2d_1 = torch.nn.functional.interpolate(feat_2d[1], size=(240, 320), mode='bicubic', align_corners=False).squeeze(0) # 1280, 240, 320
        feat_2d_2 = torch.nn.functional.interpolate(feat_2d[2], size=(240, 320), mode='bicubic', align_corners=False).squeeze(0) #  640, 240, 320
        feat_2d = torch.cat([feat_2d_0, feat_2d_1, feat_2d_2], dim=0) # 3200, 240, 320
...
def main(args):   
    ...
    # Load intrinsic parameters from scannet_2d/intrinsics.txt
    intrinsics_path = join(args.data_root_2d, 'intrinsics.txt')
